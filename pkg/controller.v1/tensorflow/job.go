...
// deleteSharePodsAndServices is SAME as deletePodsAndServices
func (tc *TFController) deleteSharePodsAndServices(tfJob *tfv1.TFJob, pods []*kubesharev1.SharePod) error {
	if len(pods) == 0 {
		return nil
	}

	/*if tfJob.Spec.CleanPodPolicy == nil {
		tmp := common.CleanPodPolicyAll
		tfJob.Spec.CleanPodPolicy = &tmp
	}*/
	...
	for _, pod := range pods {
		if *tfJob.Spec.CleanPodPolicy == common.CleanPodPolicyRunning && (pod.Status.PodStatus == nil || pod.Status.PodStatus.Phase != v1.PodRunning) {
			continue
		}
