...
// FilterActiveSharePods is SAME as FilterActivePods
func FilterActiveSharePods(pods []*kubesharev1.SharePod) []*kubesharev1.SharePod {
	var result []*kubesharev1.SharePod
	for _, p := range pods {
		if IsSharePodActive(p) {
			result = append(result, p)
		} else {
			var phase v1.PodPhase = v1.PodPending
			var del *metav1.Time = nil
			if p.Status.PodStatus != nil {
				phase = p.Status.PodStatus.Phase
			}
			if p.Status.PodObjectMeta != nil {
				del = p.Status.PodObjectMeta.DeletionTimestamp
			}
			log.Infof("Ignoring inactive pod %v/%v in state %v, deletion time %v",
				p.Namespace, p.Name, phase, del)
		}
	}
	return result
}
...
// IsSharePodActive is SAME as IsPodActive
func IsSharePodActive(p *kubesharev1.SharePod) bool {
	return (p.Status.PodStatus == nil ||
		(v1.PodSucceeded != p.Status.PodStatus.Phase && v1.PodFailed != p.Status.PodStatus.Phase)) &&
		(p.Status.PodObjectMeta == nil || p.Status.PodObjectMeta.DeletionTimestamp == nil)
}
...
// FilterSharePodCount is SAME as FilterPodCount
func FilterSharePodCount(pods []*kubesharev1.SharePod, phase v1.PodPhase) int32 {
	var result int32
	for i := range pods {
		if pods[i].Status.PodStatus != nil && phase == pods[i].Status.PodStatus.Phase {
			result++
		}
	}
	return result
